...
vector<PII > graph[ARR_ROW_MAX_LEN];
...
void make_graph()
{
    for(rint i = 1; i <= N - 1; i++){
        for(rint j = i + 1; j <= N; j++){
            rint dist = calc_hamming_distance(i - 1, j - 1);
...
bool dijkstra()
{
    ...
    while(!pq.empty()){
        PII cur = pq.top();
        rint cur_point = cur.first;
        rint cur_dist = cur.second;
        pq.pop();
        ...
        for(auto next : graph[cur_point]){
            rint next_point = next.first;
            rint next_dist = cur_dist + next.second;
...
int main(void)
{
    ...
    for(rint i = parent[B]; i != A; i = parent[i]){
    ...
    for(rint i = result_path.size() - 1; i >= 0; i--){
        cout << result_path[i] << " ";
    }
